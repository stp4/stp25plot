...
#'
#' n<-10
#' data<- data.frame(Group=gl(2, n, labels = c("Control", "Treat")),
#'                   f1=rnorm(n*2),
#'                   f2=rnorm(n*2),
#'                   f3=rnorm(n*2),
#'                   f4=rnorm(n*2),
...
#'
#' library(stp25)
#' x<-Recast2(.~Group, data, function(x) mean(x))
#' x$variable<- factor(x$variable, levels(x$variable), labels)
#'
#'
#'
#' windows(8,8)
#' profile.plot( tidyr::spread(x, Group, value) ,
#'               draw.mid=FALSE
#' )
profile.plot <- function(
  ...
  if(is.null(text.left) & is.null(text.right)){
	...
	if (plot.legend) {
		legend("bottom", legend=colnames(x), pch=19,
		       col=colors, bg=NA, box.col=NA,
		       ncol=legend.n.col)
	}
