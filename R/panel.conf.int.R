...
#'
#'
panel.conf.int <- function(x, y, ...,  pch = 16, conf.level = 0.95, length = 0.05, method ="mean") {
  if(any(class(x)=="factor"))  {
    ...
    if(method =="mean" || method ==1){
      m_x <- tapply(y, factor(x), FUN = mean, na.rm=TRUE)
      Cis<-sapply(split(y,x), function(a) t.test(a, conf.level = conf.level )$conf.int  )
    }else{ #Median
      m_x <- tapply(y, factor(x), FUN = median, na.rm=TRUE)
      Cis<- sapply(split(y,x),
                   function(a){  #-- Fehler bei Wenigen Daten
                     res_ci <- sort(a)[qbinom(c(.025,.975), length(a), 0.5)]
                     if(length(res_ci)==1) res_ci <-(range(a))
                     return(res_ci)}
      )
    }
    panel.points(y_new, m_x, pch = pch, ...)
    panel.arrows(y_new,Cis[1,],  y_new,  Cis[2,],lty=1, angle = 90, code=3, length = length)
    ...
    if(method =="mean" || method ==1){
      ...
      Cis<-sapply(split(x,y), function(a) t.test(a, conf.level = conf.level )$conf.int  )
    }else{ #Median
    ...
    panel.arrows(Cis[1,], y_new, Cis[2,], y_new, lty=1, angle = 90, code=3, length = length)
  }
