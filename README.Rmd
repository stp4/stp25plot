...
DF_sprk1 <- stp25aggregate::Summarise(DF_sprk, x~ Laborwert+ Time, fun=mean )
names(DF_sprk1)[4]<- "x"
#DF_sprk<- DF_sprk[-3]
```
...
grid.arrange(plot(eff), g, 
             xyplot(y~education, dat),
             plot(mydf),
             ncol = 2)
...
```{r effect-ggplot, fig.cap='Effect ggplot',fig.height=3, fig.width= 5}
 
Model <- lm(drat~hp*cyl, data=mtcars)


ef <- effects::effect(term = "hp:cyl", Model, default.levels = 9) # 9 because the breaks are nicer
